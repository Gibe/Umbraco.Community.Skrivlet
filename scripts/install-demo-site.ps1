...
$csprojPath = Join-Path $RepoRoot "Umbraco.Community.Skrivlet\SkrivLet.csproj"
if (-not (Test-Path $csprojPath)) {
    Write-Host "ERROR: Could not find $csprojPath" -ForegroundColor Red
    exit 1
}
$csprojContent = Get-Content $csprojPath -Raw
if ($csprojContent -match 'Umbraco\.Cms\.Core"\s+Version="([^"]+)"') {
    $TemplateVersion = $matches[1]
} else {
    Write-Host "ERROR: Could not find the Umbraco.Cms.Core version in $csprojPath" -ForegroundColor Red
    exit 1
}
...
$LibraryProject = "Umbraco.Community.Skrivlet\SkrivLet.csproj"
